...
function TransactionHistory() {
  ...
  const enterBulkEditMode = () => {
    ...
    const parsedTransactions = filteredTransactions.map(
      convertTransactionToParsed,
    );
    ...
    const enhancedGroups = groups.map((group) => {
      ...
      const updatedTransactions = group.transactions.map((t) => {
        const originalTransaction = filteredTransactions.find(
          (orig) => orig.id === t.id,
        );
        return {
          ...t,
    ...
    const groupedIds = new Set(
      enhancedGroups.flatMap((g) => g.transactions.map((t) => t.id)),
    );
  ...
  const updateIndividualTransaction = (
        ...
        <div className="summary-and-actions">
          {filteredTransactions.length > 0 && (
            <div className="filter-summary">
              ...
              <div
                className={`summary-item balance ${totalIncome - totalExpenses >= 0 ? "positive" : "negative"}`}
              >
      ...
      <div className="card">
        {isLoading ? (
          <div className="empty-state">
            ...
            <p>
              Processing your transaction history. This may take a moment for
              large datasets.
            </p>
          </div>
        ) : filteredTransactions.length === 0 ? (
          ...
          <div className="bulk-edit-content">
                        ...
                        {!showNewCategoryInputs.get(groupIndex) ? (
                          ...
                          <div className="new-category-input-group">
                            ...
                            <button
                              ...
                              disabled={
                                !newCategoryNames.get(groupIndex)?.trim()
                              }
                            >
                              <PlusCircle size={14} />
                              Add
            ...
            {ungroupedTransactions.length > 0 && (
              <>
                <div className="groups-summary">
                  <h3>
                    Individual Transactions ({ungroupedTransactions.length})
                  </h3>
                ...
                {ungroupedTransactions.map((transaction) => (
                  <div
                    key={transaction.id}
                    className="transaction-group individual-transaction"
                  >
                    <div className="group-header">
                      <div className="group-info">
                        <div className="group-title-section">
                          ...
                          <span className="group-count">
                            {formatCurrency(transaction.amount)} on{" "}
                            {format(
                              parseLocalDate(transaction.date),
                              "MMM dd, yyyy",
                            )}
                      ...
                      <div className="group-controls">
                        <select
                          ...
                          onChange={(e) =>
                            updateIndividualTransaction(transaction.id, {
                              type: e.target.value as "income" | "expense",
                            })
                          }
                          aria-label="Transaction type"
