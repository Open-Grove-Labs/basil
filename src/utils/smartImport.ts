    ...
    try {
      ...
      const type = determineTransactionType(
        ...
        columnMapping.typeColumn
          ? String(row[columnMapping.typeColumn])
          : undefined,
        isDebit,
      );
      ...
      const transaction: ParsedTransaction = {
        ...
        category: columnMapping.categoryColumn
          ? String(row[columnMapping.categoryColumn] || "")
          : "",
        type,
        createdAt: columnMapping.createdAtColumn
          ? String(row[columnMapping.createdAtColumn] || "")
          : undefined,
        confidence: calculateParsingConfidence(
          dateStr,
          descriptionStr,
          String(amount),
        ),
      ...
      if (isDuplicate) {
        ...
        transaction.duplicateReason =
          "Similar transaction found (date, amount, description)";
      }
...
export function groupTransactionsByDescription(
  ...
  for (const transaction of transactions) {
    ...
    const groupKey = transaction.isDuplicate
      ? `${normalized}__DUPLICATE`
      : normalized;
...
function getMostCommonType(
  ...
  return Object.entries(typeCounts).reduce((a, b) =>
    typeCounts[a[0] as keyof typeof typeCounts] >
    typeCounts[b[0] as keyof typeof typeCounts]
      ? a
      : b,
  )[0] as "income" | "expense";
}
