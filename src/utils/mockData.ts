...
function generateMockTransactions(): Transaction[] {
  ...
  for (let month = 0; month < 6; month++) {
    ...
    if (Math.random() > 0.7) {
      const template =
        INCOME_TEMPLATES[
          Math.floor(Math.random() * (INCOME_TEMPLATES.length - 1)) + 1
        ];
      const date = new Date(
        currentMonth.getFullYear(),
        currentMonth.getMonth(),
    ...
    for (let i = 0; i < expenseCount; i++) {
      const template =
        EXPENSE_TEMPLATES[Math.floor(Math.random() * EXPENSE_TEMPLATES.length)];
      const date = new Date(
        currentMonth.getFullYear(),
        currentMonth.getMonth(),
        Math.floor(Math.random() * 28) + 1,
...
export function loadMockData(): void {
  ...
  console.log(
    `✅ Generated ${mockTransactions.length} mock transactions across 6 months`,
  );
  ...
  console.log("");
  console.log(
    '💡 To remove mock data, run: localStorage.removeItem("basil.addMockData")',
  );
}
...
export function initializeMockData(): void {
  if (shouldLoadMockData()) {
    ...
    const existingTransactions = JSON.parse(
      localStorage.getItem("basil_transactions") || "[]",
    );
  ...
  window.basilBudget = {
    ...
    reloadMockData: () => {
      ...
      if (shouldLoadMockData()) {
        loadMockData();
        console.log(
          "🔄 Mock data reloaded! Refresh the page to see updated data.",
        );
      } else {
        console.log(
          "❌ Mock data flag not set. Run basilBudget.addMockData() first.",
        );
      }
    },
  };
